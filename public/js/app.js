...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  created: function created() {
    ...
    this.loading = true; // 1. Check if review already exists (in reviews table by id - uuid)

    axios.get("/api/review/".concat(this.$route.params.id)).then(function (response) {
      _this.existingReview = response.data.data;
    })["catch"](function (error) {// console.log(`Error! ${error.response.status} ${error.response.statusText}`)
    })["finally"](function () {
      return _this.loading = false;
...
window.Vue = __webpack_require__(/*! vue */ "./node_modules/vue/dist/vue.esm.js").default; // Register component globally

Vue.component('star-rating', _shared_components_StarRating__WEBPACK_IMPORTED_MODULE_0__.default); // Register filter globally
